        ...
        if(ctdaemon::checkTimer($this->timer_sync_trade_pairs, $this->timer_sync_trade_pairs_ts)) {
            ...
            if(is_array($exchanges) && !empty($exchanges)) {
                foreach ($exchanges as $exch) {
                    ...
                    if($exch_obj->getId() < 1) {
                        Log::systemLog('error', 'Error create Exchange object at Sync spot trade pairs');
                    }
                    else {
                        $status_sync_pair = $exch_obj->syncSpotAllTradePair();
                        if($status_sync_pair) {
                            Log::systemLog('debug', 'Process "Service" pid='. getmypid().'. Exchange\'s trade pairs '.$exch_obj->getName().' is syncronised');
                        }
                    }
                    unset($exch_obj);
                }
                ...
                foreach ($list as $l) {
                    ...
                    if($fee) {
                        ...
                        if(!empty($DB->getLastError())) {
                            Log::systemLog("error", 'Error insert trade pair FEE Exchange='.$l['EX_ID'].' Account_id='.$l['EA_ID']);
                        }
                        else {
                            Log::systemLog("debug", 'Trade pair\'s Fee Exchange='.$exchange->getName().' Account_id='.$l['EA_ID'].' Pair='.Exchange::detectNamesPair($l['PAIR_ID']) .' is updated');
                        }
                    }
                    $last_ex = $l['EX_ID'];
                    usleep(200000);
           ...
           /* $sql = "WITH
            ...
            if(is_array($exch_update) && !empty($exch_update)) {
                foreach ($exch_update as $eu) {
                    ...
                    Log::systemLog("debug", "Echange ".$exchange->getName()." update coins info is finished");
                    unset($exchange);
                }
